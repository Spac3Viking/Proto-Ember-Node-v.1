...
describe('Phase 17B — Canonical Cache Package Structure', () => {
    ...
    test('non-core install enforces documents and artifact cache layers', async () => {
        ...
        expect(fs.existsSync(path.join(sc.ARCHIVE_CACHES_DIR, 'green-fire-codices-cache', 'documents'))).toBe(true);
        expect(fs.existsSync(path.join(sc.ARCHIVE_CACHES_DIR, 'green-fire-codices-cache', 'artifacts'))).toBe(true);
        expect(fs.existsSync(path.join(sc.ARCHIVE_CACHES_DIR, 'green-fire-codices-cache', 'README.md'))).toBe(true);
        expect(fs.existsSync(path.join(sc.ARCHIVE_CACHES_DIR, 'green-fire-codices-cache', 'documents', 'bootstrap.md'))).toBe(true);
        expect(fs.existsSync(path.join(sc.ARCHIVE_CACHES_DIR, 'green-fire-codices-cache', 'artifacts', 'source.pdf'))).toBe(true);
    });

    test('archive cache detection is documents-first by default', () => {
        const sc = require('../app/storageConfig');
        sc.ensureDataRoot();
        ...
        expect(defaultPaths.some(p => p.endsWith(path.join('documents', 'summary.md')))).toBe(true);
    ...
    test('non-core install rejects legacy continuity/ cache layer packages', async () => {
        const { app } = require('../app/server');
        const { ARCHIVE_ENDPOINTS } = require('../app/archiveCacheService');
        const cacheZip = new AdmZip();
        cacheZip.addFile('green-fire-codices-cache/manifest.json', Buffer.from(JSON.stringify({
        ...
        cacheZip.addFile('green-fire-codices-cache/README.md', Buffer.from('# Legacy Continuity'));
        cacheZip.addFile('green-fire-codices-cache/continuity/legacy.md', Buffer.from('# Legacy'));
        cacheZip.addFile('green-fire-codices-cache/documents/valid.md', Buffer.from('# Valid'));
        const zipBuffer = cacheZip.toBuffer();
