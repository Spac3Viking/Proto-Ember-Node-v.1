...
describe('Phase 22A — Signal Thread spiral foundation', () => {
    ...
    test('persists Field Log entries with stage and exports them to Markdown', async () => {
        const { app } = require('../app/server');
        const create = await request(app)
            .post('/api/signal-threads')
            .send({ title: 'Field Log', posture: 'reflective', currentStage: 'reflect' });
        const id = create.body.thread.id;
        ...
        expect(markdown.text).toContain('Current Stage: reflect');
